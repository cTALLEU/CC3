...
```{r}
row.names(seqtab.nochim) <- samples.out
...
labels = list(Col = list(nrow = 32)) # gives more space for the Genus names
```

```{r}
ann.dat <- data.frame(var1 = c(rep("cat1", 4), rep("cat2", 8)), var2 = rnorm(12,  mean = 50, sd = 20))

plot(annHeatmap2(as.matrix(data.prop), col = colorRampPalette(c("lightyellow", "red"), space = "rgb")(51), breaks = 50, dendrogram = list(Row = list(dendro = as.dendrogram(row.clus)), Col = list(dendro = as.dendrogram(col.clus))), legend = 3, labels = list(Col = list(nrow = 32)), ann = list(Row = list(data = ann.dat))))
```
